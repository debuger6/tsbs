...
const (
	httpClientName        = "tsbs_load_elasticsearch"
...
func NewHTTPWriter(c HTTPWriterConfig, consistency string) *HTTPWriter {
	return &HTTPWriter{
		...
		c:   c,
		url: []byte(c.Host + "/_bulk"),
	}
}

var (
	methodPost = []byte("POST")
	appJson    = []byte("application/json")
)

func (w *HTTPWriter) initializeReq(req *fasthttp.Request, body []byte, isGzip bool) {
	req.Header.SetContentTypeBytes(appJson)
...
func (w *HTTPWriter) WriteRows(body []byte, isGzip bool) (int64, error) {
